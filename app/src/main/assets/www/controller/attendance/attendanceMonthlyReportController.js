colorAdminApp.controller('attendanceMonthlyReportController', function ($scope, $rootScope, $http, $state) {
    ...
    $scope.getAttendanceMonthlyReport = function () {
        ...
        $http.get($scope.webApiUrl + 'teacher/GetAttendanceMonthlyReport?apiKey=' + $scope.school.appKey + '&employeeId=' + $scope.teacher.employeeId + "&academicYearId=" + $scope.academicYear.academicYearId +
           '&month=' + $scope.selectedMonth + "&applicationTypeId=" + $scope.applicationTypeId + "&version=" + $scope.version).success(function (result) {
               $scope.attendanceDays = [];
